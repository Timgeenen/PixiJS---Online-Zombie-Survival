...
function LobbyListContainer() {
    ...
    function handleClick(e: React.MouseEvent<HTMLDivElement>) {
        ...
        if (!action || !lobby_id) { return }
        if (!!!isprivate) { console.log("HANDLE PRIVATE ROOM")};
    ...
    const lobbies = Array.from(lobbyMap.values()).map((lobby) => (
        <div key={lobby._id} className='flex items-center'>
            ...
            <span className='w-8'>{lobby.currentPlayers}/{lobby.settings.maxPlayers}</span>
            <span className='w-8'>{lobby.settings.isPrivate && 'Private'}</span>
            <button data-action='join' data-lobby_id={lobby._id} data-isprivate={lobby.settings.isPrivate} className='border p-1'>Join</button>
        </div>
    ));
